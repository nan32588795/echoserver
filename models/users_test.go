...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Username`: `character varying`, `Email`: `character varying`, `PasswordHash`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
